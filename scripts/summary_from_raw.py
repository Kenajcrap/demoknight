...
def main(argv):
    ...
    for t in range(2):
            entry = {"name": f"Test_{t}", "Average Frametime": [], "Variance of Frametime": []}
            ...
            for i in range(int(len(raw_files)/2)*t,int(len(raw_files)/2)*(t+1)):
                if i in (0, 1200):
                    continue
                arr = np.loadtxt(raw_files[i], delimiter=",", usecols=usecols, skiprows=skiprows)
    ...
    with open(
            f"{output_file}.{format}", "w", newline="", encoding="utf-8"
        ) as outfile:
            if format == "json":
